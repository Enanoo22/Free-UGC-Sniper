    ...
    class Sniper:
        ...
        def __init__(self) -> None:
            ...
            async def user_disconnected(data, self):
                if data.get("room_code", "kekw") != self.config.get("rooms", {}).get(
                    "room_code"
                ):
                    return
            ...
            async def new_roblox_item(self, data):
                ...
                if self.config.get("rooms", {}).get("item_setup", {}).get(
                    "max_price"
                ) is not None and int(data.get("price", 0)) > self.config.get(
                    "rooms", {}
                ).get(
                    "item_setup", {}
                ).get(
                ...
                coroutines = [
                    self.buy_item(
                        item_id=data["data"]["CollectibleItemId"],
            ...
            async def new_item(self, data):
                ...
                coroutines = [
                    self.buy_item(
                        item_id=data["collectibleItemId"],
                        price=data["price"],
                        user_id=self.accounts[i]["id"],
        ...
        @property
        def rooms(self):
            return True if self.config.get("rooms", {}).get("enabled", None) else False

        @property
        def room_code(self):
            return self.config.get("rooms", {}).get("room_code", None)

        @property
        def username(self):
            return self.config.get("rooms", {}).get("username", None)

        @property
        def webhookEnabled(self):
            return (
                True if self.config.get("webhook", {}).get("enabled", None) else False
            )

        @property
        def webhookUrl(self):
            return self.config.get("webhook", {}).get("url", None)
        ...
        @property
        def themeWaitTime(self):
            return float(self.config.get("theme", {}).get("wait_time", 1))
        ...
        def timeout(self):
            return (
                self.config.get("proxy", {}).get("timeout_ms", None) / 1000
                if self.config.get("proxy", {}).get("enabled", None)
                else None
            )
        ...
        def run_bot(self):
            ...
            async def stats(ctx):
                ...
                embed.add_field(
                    name="Loaded Items", value=f"{len(self.items)}", inline=True
                )
                ...
                embed.add_field(
                    name="Last Speed", value=f"{self.last_time}", inline=True
                )
                embed.add_field(
                    name="Total Price Checks", value=f"{self.checks}", inline=True
                )
                if self.rooms:
                    embed.add_field(
                        name="Room Users", value=f"{len(self.users)}", inline=True
                    )
                    embed.add_field(
                        name="Room Code", value=f"{self.room_code}", inline=True
                    )
            ...
            async def remove_id(ctx, arg=None):
                if arg is None:
                    return await ctx.reply(
                        ":warning: | You need to enter an ID to remove!"
                    )
                ...
                if not arg in self.items:
                    return await ctx.reply(
                        ":grey_question: | ID is not curently running."
                    )
            ...
            async def add_id(ctx, id=None, max_price=None, max_buys=None):
                if id is None:
                    return await ctx.reply(
                        ":warning: | You need to enter an ID to add!"
                    )
                ...
                for item in self.config["items"]["item_list"]:
                    if int(item["id"]) == int(id):
                        item = item
                        break
                self.items[id]["max_buys"] = (
                    float("inf") if item["max_buys"] is None else int(item["max_buys"])
                )
                self.items[id]["max_price"] = (
                    float("inf")
                    if item["max_price"] is None
                    else int(item["max_price"])
                )
            ...
            async def link(ctx, id=None):
                if id is None:
                    return await ctx.reply(
                        ":warning: | You need to enter an ID or position to get!"
                    )
                ...
                if id in self.items:
                    ...
                    await ctx.reply(
                        f"https://www.roblox.com/catalog/{keys[int(id) - 1]}"
                    )
                else:
                    return await ctx.reply(f":x: | Invalid given ID or position: {id}")
        ...
        def check_version(self):
            ...
            response = requests.get(
                "https://raw.githubusercontent.com/CenvoldYT/Free-UGC-Sniper/main/version"
            )
        ...
        def _load_items(self) -> list:
            ...
            for item in self.config["items"]["item_list"]:
                ...
                dict[item["id"]]["max_buys"] = (
                    float("inf") if item["max_buys"] is None else int(item["max_buys"])
                )
                dict[item["id"]]["max_price"] = (
                    float("inf")
                    if item["max_price"] is None
                    else int(item["max_price"])
                )
        ...
        async def _get_user_id(self, cookie) -> str:
            async with aiohttp.ClientSession(
                ...
                response = await client.get(
                    "https://users.roblox.com/v1/users/authenticated", ssl=False
                )
        ...
        async def _get_xcsrf_token(self, cookie) -> dict:
            ...
            async with aiohttp.ClientSession(
                ...
                response = await client.post(
                    "https://accountsettings.roblox.com/v1/email", ssl=False
                )
                xcsrf_token = response.headers.get("x-csrf-token")
                if xcsrf_token is None:
                    raise Exception(
                        "An error occurred while getting the X-CSRF-TOKEN. "
        ...
        async def _check_xcsrf_token(self) -> bool:
            for i in self.accounts:
                if self.accounts[i][
                    "xcsrf_token"
                ] is None or datetime.datetime.now() > (
                    self.accounts[i]["created"] + datetime.timedelta(minutes=10)
                ):
                    try:
                        response = await self._get_xcsrf_token(
                            self.accounts[i]["cookie"]
                        )
        ...
        async def buy_item(
            ...
            async with aiohttp.ClientSession() as client:
                while True:
                    if self.items.get(raw_id, {}).get("max_buys", 0) != 0 and not float(
                        self.items.get(raw_id, {}).get("max_buys", 0)
                    ) >= float(self.items.get(raw_id, {}).get("current_buys", 1)):
                        del self.items[id]
                        for item in self.config["items"]["item_list"]:
                    ...
                    try:
                        response = await client.post(
                            f"https://apis.roblox.com/marketplace-sales/v1/item/{item_id}/purchase-item",
                            json=data,
                    ...
                    if not json_response["purchased"]:
                        ...
                        if self.webhookEnabled:
                            embed_data = {
                                ...
                                "color": 65280,
                                "author": {"name": "SNIPED FREE LIMITED"},
                                "footer": {"text": "Cenvold's UGC Sniper"},
                            }

                            requests.post(
                                self.webhookUrl,
                                json={"content": None, "embeds": [embed_data]},
                            )
        ...
        async def search(self, session) -> None:
            ...
            while True:
                try:
                    ...
                    currentAccount = self.accounts[
                        str(random.randint(1, len(self.accounts)))
                    ]
                    async with session.post(
                        "https://catalog.roblox.com/v1/catalog/items/details",
                        json={
                            "items": [{"itemType": "Asset", "id": id} for id in items]
                        },
                        headers={
                        ...
                        for i in json_response:
                            if int(i.get("price", 0)) > self.items[id]["max_price"]:
                                ...
                                continue
                            if (
                                i.get("priceStatus") != "Off Sale"
                                and i.get("unitsAvailableForConsumption", 0) > 0
                            ):
                                await self.ratelimit.take(
                                    1,
                                    proxy=True
                                ...
                                productid_response = await session.post(
                                    ...
                                    cookies={
                                        ".ROBLOSECURITY": currentAccount["cookie"]
                                    },
                                    ssl=False,
                                )
                                response.raise_for_status()
                                productid_data = json.loads(
                                    await productid_response.text()
                                )[0]
                                self.totalTasks += 1
                                coroutines = [
                                    self.buy_item(
                                        item_id=i["collectibleItemId"],
                                        price=i["price"],
                                        user_id=self.accounts[o]["id"],
                                        creator_id=i["creatorTargetId"],
                                        product_id=productid_data[
                                            "collectibleProductId"
                                        ],
                                        cookie=self.accounts[o]["cookie"],
                                        x_token=self.accounts[o]["xcsrf_token"],
                                        raw_id=id,
                                    )
                    ...
                    async with aiohttp.ClientSession() as client:
                        for i in range(self.total_to_search):
                            async with client.get(
                                f"{self.config['items']['auto_search']['search_url']['endpoint']}&cursor={current_curser}"
                                if not current_curser is None
                                else f"{self.config['items']['auto_search']['search_url']['endpoint']}",
                                headers={
                                ...
                                for item in json["data"]:
                                    if (
                                        item.get("priceStatus", "nyet") == "Off Sale"
                                        and not "price" in item
                                        and not item["id"] in self.items
                                    ):
                                        self.items[str(item["id"])] = {
                                            "current_buys": 0,
                                            "max_buys": float("inf"),
                                            "max_price": float("inf")
                                            if self.config["items"]["auto_search"][
                                                "max_price"
                                            ]
                                            is None
                                            else self.config["items"]["auto_search"][
                                                "max_price"
                                            ],
                                        }
                                await asyncio.sleep(5)
                            self.items = {
                                str(key): value for key, value in self.items.items()
                            }
        ...
        async def start(self):
            ...
            if self.rooms:
                await sio.connect(
                    "https://electroniclightpinkfunnel.rfrrgf.repl.co",
                    headers={"room": self.room_code, "user": self.username},
                )
        ...
        async def auto_xtoken(self):
            while True:
                ...
                ), "x_csrf_token couldn't be generated"
